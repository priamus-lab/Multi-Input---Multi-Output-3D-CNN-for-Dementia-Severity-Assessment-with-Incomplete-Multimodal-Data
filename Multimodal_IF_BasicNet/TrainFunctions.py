...
def train_loop_validation(model_conv, trainset_mri, trainset_pet, trainset_paired,
  ...
  for epochs in range(start, num_epoch + 1):
    ...
    for i in range(0, niter):
      ...
      mri_input = mri_input.type(torch.FloatTensor).to(device=device)
      pet_input = pet_input.type(torch.FloatTensor).to(device=device)
...
def ValidationStep(Val_mri, Val_pet, Val_paired, loader_opts, model_conv, criterionCNN, device, outputPath):
  ...
  for mri_input, pet_input, labels in ValLoader_paired:
    ...
    modelLoss_val += lossCNN.item() * mri_input.size(0)  #Non pesata -> semplice media
    totalSize_val += mri_input.size(0)
    modelAcc_val += torch.sum(preds == labels.data).item()
...
def validateOnTest(Test_mri, Test_pet, Test_paired,start, loader_opts, model_conv, criterionCNN, device):
  ...
  for mri_input, labels in TestLoader_mri:
    ...
    model_loss_test += lossCNN.item() * mri_input.size(0)  #Non pesata -> semplice media
  ...
  for pet_input, labels in TestLoader_pet:
    ...
    model_loss_test += lossCNN.item() * pet_input.size(0)  #Non pesata -> semplice media
...
## #-------------------------------------------------------------------------------> Predict function
def prediction_on_Test(model_conv, Test_mri, Test_pet, Test_paired, transform_mri,transform_pet, readerMRI, readerPET, device):
  ...
  for path, label_true in testFiles:
    ...
    inputs_mri = inputs_mri.type(torch.FloatTensor).unsqueeze(0).to(device=device)
    inputs_pet = inputs_pet.type(torch.FloatTensor).unsqueeze(0).to(device=device)
...
def Main_train(vali_set, test_set, patient_to_include, listClasses, newClasses, basePath_mri, basePath_pet, basePath_Paired, ch, batchSize,
  ...
  model_conv = MyNet_Multimodal(1, ch, num_classes, model_type, path_mri, path_pet, False)
  ...
  tabella_both, tabella_mri, tabella_pet = prediction_on_Test(model_conv, test_mri_dataset, test_pet_dataset, test_paired_dataset, val_transform_mri,val_transform_pet, loader_mri, loader_pet, device)
