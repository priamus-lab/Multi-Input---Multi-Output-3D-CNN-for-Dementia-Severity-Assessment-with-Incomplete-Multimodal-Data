...
class My_DatasetFolder(Dataset):
  def __init__(self, root,  transform, is_valid_file, list_classes, readVolume, type_i):
    self.root = root 
    ...
    for c in self.list_classes:
      ...
      for file in listofFiles:
        if self.is_valid_file(self.root + '/' + c + '/' + file):
          if c == self.list_classes[0]:  #se è NORM
            ListFiles.append((self.root + '/' + c + '/' + file, 0))   
          else:
            ListFiles.append((self.root + '/' + c + '/' + file, 1))   
    return ListFiles
  ...
  def __getitem__(self, index: int):
    ...
    empty = torch.zeros(volume.shape)    
    if self.type_i == 'mri':
      volume_finale = torch.cat((volume, empty), dim = 0)
    else:
      volume_finale = torch.cat((empty, volume), dim = 0)

    return volume_finale, target    
'''
class My_DatasetFolderMultiModal(Dataset):
  ...
  def __getitem__(self, index: int):
    ...
    volume_finale = torch.cat((volume_mri, volume_pet), dim = 0)
    return volume_finale, target
