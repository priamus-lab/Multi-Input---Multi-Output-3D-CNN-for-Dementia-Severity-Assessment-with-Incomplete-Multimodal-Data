...
def train_loop_validation(model_conv, 
                          big_dataset, small_dataset, Val, test, 
                          start, num_epoch, 
                          loader_opts, 
  ...
  for epochs in range(start, num_epoch + 1):
    ...
    TrainLoader_bigger = DataLoader(big_dataset, shuffle=True, **loader_opts)
    TrainLoader_smaller = DataLoader(small_dataset, shuffle=True, **loader_opts)
    p_small = iter(TrainLoader_smaller)
    ...
    #-----------------------------------------------------------------------------> TRAIN      
    for inputs_bigger, labels_bigger in TrainLoader_bigger:
      
      try:
        inputs_small, labels_small = p_small.next() 
      except:
        TrainLoader_smaller = DataLoader(small_dataset, shuffle=True, **loader_opts)
        p_small = iter(TrainLoader_smaller)
        inputs_small, labels_small = p_small.next()
      ...
      inputs_bigger = inputs_bigger.type(torch.FloatTensor)
      inputs_small = inputs_small.type(torch.FloatTensor)
      
      inputs = torch.cat((inputs_bigger, inputs_small), dim=0).to(device=device)
      labels = torch.cat((labels_bigger, labels_small), dim=0).to(device=device)
    ...
    print('[Epoch %d][TRAIN on %d [Loss: %.4f - ACC_T: %.4f - ACC_0: %.4f - ACC_1: %.4f]]' %(epochs, totalSize, modelLoss_epoch_train, modelAcc_epoch_train, acc_0_T, acc_1_T))
    #-----------------------------------------------------------------------------> VALIDATION 
    model_conv.eval()    
...
def ValidationStep(Val, loader_opts, model_conv, criterionCNN, device, outputPath):
  ...
  w1 = (2*n0)/(n0+n1)
  w0 = (2*n1)/(n0+n1) 
  mean_accuracy_val = (acc_1_V + acc_0_v)/2
  ...
  print('    [VAL on %d [w0:%.4f - w1:%.4f][Loss: %.4f - ACC_T: %.4f - ACC_0: %.4f - ACC_1: %.4f - ACC_Mean: %.4f - ACC_weig: %.4f]]' 
    %(totalSize_val, w0, w1, modelLoss_epoch_val, modelAcc_epoch_val, acc_0_v, acc_1_V, mean_accuracy_val, weight_mean_val))
...
def validateOnTest(test, loader_opts, model_conv,criterionCNN, device):
  ...
  for  inputs, labels in TestLoader:
    ...
    model_loss_test += lossCNN.item() * inputs.size(0)  
...
def Main_train(vali_set, test_set, patient_to_include, listClasses, newClasses, basePath_mri, basePath_pet, basePath_Paired,
               ch, batchSize,
               outputPath, num_epoch, learningRate, weightDecay, continue_learning, model_type, resize,  
  ...
  big_dataset = dataset_paired[0]
  small_dataset = dataset_paired[1]
  small_dataset.samples += dataset_paired[2].samples
  ...
  model_conv = MyNet_Simple(2,ch,num_classes, False)
  ...
  model_conv = train_loop_validation(model_conv, 
                          big_dataset, small_dataset, val_paired_dataset, test_paired_dataset, 
